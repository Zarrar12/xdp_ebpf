...
// default attached secion
// static const char *tail_progsec = "xdp_1";
static const char *tail_progsec = "xdp_1";
const char *map_name = "newTable";
...
struct bpf_object *__load_bpf_and_xdp_attach(struct config *cfg)
{
	...
	// bpf_prog = bpf_program__section_name(cfg->progsec);
	if (!bpf_prog) {
...
int pin_maps_in_bpf_object(struct bpf_object *bpf_obj, const char *subdir)
{
	...
	if (access(map_filename, F_OK ) != -1 ) {
		...
		err = bpf_object__unpin_programs(bpf_obj, pin_dir);
		if (err) {
			fprintf(stderr, "ERR: UNpinning progs in %s\n", pin_dir);
	...
	err = bpf_object__pin(bpf_obj, pin_dir);
	if (err){
		fprintf(stderr, "ERR: Can't pin bpf_obj\n");
		return EXIT_FAIL_BPF;
	}
...
static int fill_jmpTable_idx (struct bpf_object *bpf_obj, struct config *cfg)
{
	struct bpf_program *bpf_prog;
	int prog_fd = -1;
	int map_fd = -1, map_val = 0;
	int idx = 0, err = 0;

	/* Find a matching BPF prog section name */
	bpf_prog = bpf_object__find_program_by_title(bpf_obj, tail_progsec);
	if (!bpf_prog) {
		fprintf(stderr, "ERR: finding progsec: %s\n", tail_progsec);
		exit(EXIT_FAIL_BPF);
	}

	// Get the fd for the obtained tail call section
	prog_fd = bpf_program__fd(bpf_prog);
	if (prog_fd <= 0) {
		fprintf(stderr, "ERR: bpf_program__fd failed\n");
		exit(EXIT_FAIL_BPF);
	}
	printf ("Got prog_fd of %s: %d\n", tail_progsec, prog_fd);

	// Now obtain the jmpTable map_fd and fill it's index with the prog_fd
	// map_fd =  find_map_fd(bpf_obj, "scratchPad");
	map_fd =  find_map_fd(bpf_obj, map_name);
	if (map_fd < 0) {
		xdp_link_detach(cfg->ifindex, cfg->xdp_flags, 0);
		printf ("Unable to get %s fd\n", map_name);
	...
	// Now updating jmp_table1
	// err = bpf_map_update_elem(map_fd, &idx, &prog_fd, 0);
	// bpf_map_lookup_elem(fd, &key, value)
	err = bpf_map_lookup_elem(map_fd, &idx, &map_val);
	if(err) {
...
int main(int argc, char **argv)
{
	...
	// I need to attach my function here which will shall
	// update the prog_array with the tail call prog_fd 
